...
die "Usage: g03gen GaussFileName [NumNodes]" unless (@ARGV >= 1);
...
print SCRIPT "date\n", 
	"cd \$GAUSS_SCRDIR\n",  
	"time $g03exe <\$GAUSS_USER/$gaussFile &> \$GAUSS_USER/\$GAUSS_LOG  \n", 
	"date\n";
...
if [ \$? ]; then
  ...
  export g03error="Could not copy files from \$GAUSS_SCRDIR to \$GAUSS_USER"
  echo \$g03error
fi
